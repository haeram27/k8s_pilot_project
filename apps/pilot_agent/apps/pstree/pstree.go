...
func New(proc_path string) (*Tree, error) {
	files, err := filepath.Glob(proc_path + "/[0-9]*")
	if err != nil {
	...
	for _, dir := range files {
		...
		rfile, _ := os.Open(dir + "/cmdline")
		scanner := bufio.NewScanner(rfile)
		for scanner.Scan() {
			proc.CmdLine = scanner.Text()
		}
		defer rfile.Close()
...
func ProcScan(dir string) (Process, error) {
